...
def _validate_level(level: int) -> int:
    if not isinstance(level, int) or level < 50 or level > 99:
        raise ValueError("level must be an integer between 50 and 99.")
    return level
...
def _validate_requested_leads(lead: int | Sequence[int]) -> tuple[int, ...]:
    ...
    for current_lead in parsed:
        ...
        if current_lead in requested:
            raise ValueError("lead sequence must not contain duplicate values.")
        requested.append(current_lead)
    if not requested:
        raise ValueError("lead must contain at least one non-negative lead.")
    return tuple(requested)


def _validate_single_lead(value: Any) -> int:
    if isinstance(value, bool) or not isinstance(value, (int, np.integer)):
        raise ValueError("lead must be an integer or a sequence of integers.")
    return int(value)
...
def _validate_se_boot(se_boot: bool | None, *, design: str) -> bool:
    ...
    if not _is_bool_like(se_boot):
        raise ValueError("se_boot must be a boolean.")
...
def _validate_thres(thres: int | None, *, design: str) -> int | None:
    if design != "sa":
        if thres is not None:
            raise ValueError("thres is only supported for design='sa'.")
        return None

    if thres is None:
        return 2
    if isinstance(thres, bool) or not isinstance(thres, (int, np.integer)) or thres < 1:
        raise ValueError("thres must be an integer greater than or equal to 1 for design='sa'.")
    return int(thres)
    ...
    *,
    ...
    if data_type == "panel":
        ...
        if treat_years.empty:
            raise ValueError("At least one treated observation is required for did().")
        ...
        if treat_years.empty:
            raise ValueError("At least one post-treatment observation is required for did().")
        treat_year = float(treat_years.min())
...
def _with_outcome_delta(frame: pd.DataFrame, *, data_type: str) -> pd.DataFrame:
    ...
    raise ValueError("data_type must be 'panel' or 'rcs'.")
...
def _build_design_matrix(
    ...
    *,
    ...
    if working.empty:
        raise ValueError(f"No rows remain after filtering for {dependent}.")
...
def _solve_interaction_effect(
    ...
    *,
    ...
    if len(outcome) < matrix.shape[1]:
        raise ValueError(f"Insufficient observations to estimate {dependent}.")
    coefficients, _, rank, _ = np.linalg.lstsq(matrix, outcome, rcond=None)
    if rank < matrix.shape[1]:
        raise ValueError(f"Insufficient variation to estimate {dependent}.")
...
def _validate_kmax(kmax: int) -> int:
    ...
    if not isinstance(kmax, int) or isinstance(kmax, bool) or kmax < 1:
        raise ValueError("kmax must be an integer >= 1.")
    return kmax
...
def _identify_requested_k_leads(
    ...
    *,
    ...
    if not identified_leads:
        reason = f" Last failure: {last_failure}" if last_failure is not None else ""
        raise ValueError(f"No identifiable lead values remain for K-DID.{reason}")
    ...
    *,
    ...
    while successful_iterations < n_boot:
        ...
        if attempts > max_attempts:
            raise ValueError("Bootstrap could not produce enough valid draws for K-DID.")
...
def _compute_component_estimates(
    ...
    *,
    ...
    subset = frame.loc[frame["id_time_std"].isin((-1, lead))].copy()
    if subset["id_time_std"].nunique() != 2:
        raise ValueError("The requested lead requires identifiable last-pre and post-treatment periods.")
...
def _compute_standard_panel_components(
    ...
    *,
    ...
    if "DID" not in estimates or "sDID" not in estimates:
        raise ValueError("DID and sDID components must both be identifiable.")
    return estimates
...
def _compute_standard_panel_component_estimates(
    ...
    *,
    ...
    if treat_years.empty:
        raise ValueError("At least one treated observation is required for design(sa).")
...
def _identify_requested_sa_leads(
    ...
    *,
    ...
    if not identified_leads:
        reason = f" Last failure: {last_failure}" if last_failure is not None else ""
        raise ValueError(f"No identifiable lead values remain for did().{reason}")
    return estimates_by_lead, tuple(identified_leads), tuple(unidentified_leads)
...
def _compute_bootstrap_draws(
    ...
    *,
    ...
    while len(draws) < n_boot * len(leads):
        attempts += 1
        if attempts > max_attempts:
            raise ValueError("Bootstrap could not produce enough valid draws for DID and sDID.")
        sampled_ids = rng.randint(0, len(clusters), size=len(clusters))
        sampled_frames = []
        for draw_position, cluster_index in enumerate(sampled_ids, start=1):
            sampled_frame = clusters[cluster_index].copy()
...
def _compute_sa_bootstrap_draws(
    ...
    *,
    ...
    while successful_iterations < n_boot:
        attempts += 1
        if attempts > max_attempts:
            raise ValueError("Bootstrap could not produce enough valid draws for the SA design.")
